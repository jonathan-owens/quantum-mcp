...
for active_space_size in [4, 6, 8, 10, 12]:
    print(f"Running active space size {active_space_size}")
    for system in root_dir.glob('*.xyz'):
        xyz_fpath = system
        start_time = time.time()
        n_orb_cas = active_space_size
        nele_cas = active_space_size
        molecular_data = MolecularHamiltonian(xyz_fpath, '631g', norb_cas=active_space_size,
                                              nele_cas=active_space_size)
        ...
        for i in range(max_iter):
            ...
            if norm <= threshold:
                ...
                output_csv.write(f'{xyz_fpath.stem},{use_natural_orbits},{n_orb_cas},{nele_cas},{result_vqe.fun},{len(selected_pool)},{i},{elapsed_time}\n')
                output_csv.flush()
